...
class ConvNet(nn.Module):
    def  __init__(self, input_shape, dropout=0.2, num_classes=2) -> None:
        ...
        self.cnn = nn.Sequential(
            nn.Conv1d(in_channels=self.input_shape[1], out_channels=32, kernel_size=5, stride=2, padding='valid', bias=False),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=3),
            nn.Conv1d(in_channels=32, out_channels=64, kernel_size=5, stride=2, padding='valid', bias=False),
            nn.ReLU(),
            nn.MaxPool1d(kernel_size=3),
            nn.Dropout(dropout)
        )
